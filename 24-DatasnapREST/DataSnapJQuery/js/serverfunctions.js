...
function TServerMethods1(connectionInfo)
{
  ...
  this.Sum = function(A, B) {
    var returnObject = this.executor.executeMethod('Sum', "GET", [A, B], arguments[2], true, arguments[3], arguments[4]);
    if (arguments[2] == null) {
      if (returnObject != null && returnObject.result != null && isArray(returnObject.result)) {
        var resultArray = returnObject.result;
        var resultObject = new Object();
  ...
  this.Sum_URL = function(A, B) {
    return this.executor.getMethodURL("Sum", "GET", [A, B], arguments[2])[0];
  };
}
